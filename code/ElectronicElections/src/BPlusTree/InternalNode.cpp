...
bool InternalNode::get(char* key, VariableRecord* record)
{
	VariableRecord aux;
	bool result;
	int index = 0;
	...
	if (this->file->isCurrentLeaf())
	{
		...
		bool found = leaf.get(key, record);
	...
	result = internal.get(key, record);
	this->file->popBlock();
	return result;

}

VariableRecord* InternalNode::returnFirst(VariableRecord* r)
{
	VariableRecord aux;
	bool result;
	int index = 0;
